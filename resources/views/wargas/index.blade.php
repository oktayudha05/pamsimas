    ...
    <form action="{{ route('wargas.store') }}" method="POST" class="space-y-4"
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl p-6 border border-[#DAD887]/50 shadow-sm flex flex-col">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <!-- Form Pencarian -->
                <form method="GET" action="{{ route('wargas.index') }}" class="w-full sm:w-64">
                    <div class="relative flex items-center">
                        ...
                        @if(!empty($search))
                            <a href="{{ route('wargas.index') }}" class="absolute right-8 text-[10px] text-red-500 hover:underline mr-1">
                                Reset
                            </a>
                        @endif
                    </div>
